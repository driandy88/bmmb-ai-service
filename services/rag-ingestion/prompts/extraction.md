...
- Preserve **every figure, currency amount, percentage, ratio, date, code, and
  proper noun exactly** as written — including units, symbols, and thousands
  separators (e.g. `RM 250,000`, `3.5%`, `60 months`, `2025`). Never round,
  convert, or normalise a value.
...
- **Tables** become real Markdown tables (`| … | … |` with a header separator
  ...
  drop a column. If cells span or nest, keep the finest-grained rows you can read.
...
- If part of the page is genuinely illegible, write `[unreadable]` in its place.
  Never guess a number or word to fill a gap.
